...
Create Internet Gateway: VPC → Internet Gateways → Create Internet Gateway → Attach it to your VPC.

Create Public Route Table: VPC → Route Tables → Create → Name Public-RT → select your VPC.

Add Public Route: Public-RT → Routes → Edit routes → 0.0.0.0/0 → Target Internet Gateway.

Associate Public Subnet: Public-RT → Subnet associations → select 10.0.1.0/24 → Save.

Create NAT Gateway: VPC → NAT Gateways → Create NAT Gateway → select Public Subnet 10.0.1.0/24 → Allocate Elastic IP → Create.

Create Private Route Table: VPC → Route Tables → Create → Name Private-RT → select your VPC.

Add Private Route: Private-RT → Routes → Edit routes → 0.0.0.0/0 → Target NAT Gateway.

Associate Private Subnet: Private-RT → Subnet associations → select 10.0.2.0/24 → Save.

Launch Public Server: EC2 → Launch Instance → select VPC → Public Subnet → enable Auto-assign Public IP.

Launch Private Server: EC2 → Launch Instance → select VPC → Private Subnet → do not assign Public IP.

Public Server Internet Flow: Public Server → Public Route Table → Internet Gateway → Internet.

Private Server Internet Flow: Private Server → Private Route Table → NAT Gateway → Public Subnet → Internet Gateway → Internet.

Important: NAT Gateway must be created in the Public Subnet, not the Private Subnet.

Final Flow: Internet → IGW → Public Server and Private Server → NAT Gateway → IGW → Internet.
