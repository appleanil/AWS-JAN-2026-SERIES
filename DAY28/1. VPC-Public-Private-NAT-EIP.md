# Part 1 - Public Web Server
1. Create VPC: VPC → Create VPC → IPv4 CIDR 10.0.0.0/16 → Create VPC.

2. Create Public Subnet: VPC → Subnets → Create subnet → Select VPC → AZ ap-south-1a → CIDR 10.0.1.0/24.

3. Create Internet Gateway: VPC → Internet Gateways → Create Internet Gateway → Attach it to the VPC.

4. Create Public Route Table: VPC → Route Tables → Create → Name Public-RT → Select your VPC.

5. Add Public Route: Public-RT → Routes → Edit routes → 0.0.0.0/0 → Target Internet Gateway → Save.

6. Associate Public Subnet: Public-RT → Subnet associations → Select 10.0.1.0/24 → Save.

7. Launch Public Server: EC2 → Launch Instance → Select your VPC → Public Subnet → Enable Auto-assign Public IP.

8. Public Server Flow: Public Server → Public Route Table → Internet Gateway → Internet.

# PART 2 — PRIVATE SERVER + NAT GATEWAY

9. Create Private Subnet: VPC → Subnets → Create subnet → Select VPC → AZ ap-south-1a → CIDR 10.0.2.0/24.

10. Create NAT Gateway: VPC → NAT Gateways → Create NAT Gateway → Select Public Subnet 10.0.1.0/24 → Allocate Elastic IP → Create.

11. Create Private Route Table: VPC → Route Tables → Create → Name Private-RT → Select your VPC.

12. Add Private Route: Private-RT → Routes → Edit routes → 0.0.0.0/0 → Target NAT Gateway → Save.

13. Associate Private Subnet: Private-RT → Subnet associations → Select 10.0.2.0/24 → Save.

14. Launch Private Server: EC2 → Launch Instance → Select your VPC → Private Subnet → Disable Auto-assign Public IP.

15. Private Server Flow: Private Server → Private Route Table → NAT Gateway → Public Subnet → Internet Gateway → Internet.

16. Important: NAT Gateway must always be created inside the Public Subnet.
