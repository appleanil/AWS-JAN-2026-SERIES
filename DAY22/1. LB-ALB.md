...
"Load Balancer is used to distribute incoming application traffic across multiple backend servers. In AWS, an Application Load Balancer can route requests to registered targets through listeners and target groups. It performs health checks and routes traffic to healthy targets, which improves high availability, fault tolerance and scalability. In DevOps, we commonly use it with Auto Scaling and deployment strategies to minimize downtime."
...
# To change default port number try to changer port at listener
```
