# Video 8 and 9 are same

# Class Brief Explaination

1. Create IAM User in GUI.
2. Open in Git bash with try to access with aws configure with Access Key and Secret Key.
3. Verify with aws configure list, aws configure profiles and try to practices some more commands.
4. Create a IAM User and delete.
5. Create a S3 bucket, list bucket.
6. Create EC2 instance and create a Role with EC2 and attach to server later from server with role we can access the S3 and all.
7. Ask Chtgpt try to practices some more commands and concepts.
